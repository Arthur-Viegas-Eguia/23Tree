import java.util.*;
/**
 * Class Node models a node for a 2-3 Tree. It has methods to add nodes to
 * the tree and to print the tree to the string. Implemented as a lab for
 * Data Structures taight by professor Anya Vostinar in Carleton. Developed in
 * class.
 */
public class Node {
  ...
  */
  ...
  private static Node insertLeft(Node rt, Integer key) {
    Node newLeft = insertHelp(rt.left, key);
    if(newLeft == rt.left){
      return rt;
    } else {
      return rt.add(newLeft);
    }
  }

  private static Node insertCenter(Node rt, Integer key) {
    Node newCenterChild = insertHelp(rt.center, key);
    if(newCenterChild == rt.center){
      return rt;
    } else {
      return rt.add(newCenterChild);
    }
  }

  private static Node insertRight(Node rt, Integer key) {
    Node newRightChild = insertHelp(rt.right, key);
    if(newRightChild == rt.right){
      return rt;
    } else {
      return rt.add(newRightChild);
    }
  }
  ...
  */
  public static Node insertHelp(Node rt, Integer key) {
    if(rt == null){
      rt = new Node(key, null, null, null, null);
      return rt;
    } else if (key < rt.lval){
      return insertLeft(rt, key);
    }  else if (rt.rval == null || (key > rt.lval && key < rt.rval)){
      return insertCenter(rt, key);
    } else{
      return insertRight(rt, key);
    }
  }
  ...
  * add it to the current node, this. If this has room, newNode's value and
  * subtrees are just meshed with this node. If this does not have room, the
  ...
  * this and a new node and returned.
  *
 ...
 */
  ...
  */
  ...
  /**
   * Tests the class by adding multiple values to the tree and printing them on
   * the screen.
   */
